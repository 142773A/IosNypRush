...
class OCR: NSObject {
        ...
        let task = URLSession.shared.dataTask(with: request){ data, response, error in
            if error != nil{
                ...
                do {
                    ...
                    if results!.index(forKey: "message") != nil {
                        print("the key 'someKey' is IN the dictionary")
                             self.errorMessage = (results)!["message"] as! String
                    }
                    else{
                        self.errorMessage = ""
                    }
    ...
    func extractStringsFromDictionary(_ dictionary: [String : AnyObject]) -> [String] {
        ...
        self.finalExtractedText = ""
        if(self.errorMessage == ""){
            do {
                
                // Get Regions from the dictionary
                let regions = try ((dictionary["regions"] as! NSArray).firstObject as? [String:AnyObject])
                ...
                if (regions != nil) {
                    ...
                    print (inLine)
                    // Get text from words //logic wrong here
                    // let extractedText = inLine.enumerated().map { $0.element[]["text"] as! String}
                    
                    extractedText = inLine.enumerated().map { (index, element) -> String in
                        var textExtract = ""
                        print(element.count)
                        ...
                        for e in element{
                            for (key, value) in e {
                                ...
                                if(key == "text"){
                                    ...
                                    textExtract = textExtract + (value as! String).lowercased()
                                }
                        ...
                        finalExtractedText = finalExtractedText + "\(textExtract)"
                        return "\(textExtract)"
                    }
    ...
    func extractStringFromDictionary(_ dictionary: [String:AnyObject]) -> String {
        ...
        if(stringArray.count > 0){
            reducedArray = stringArray.enumerated().reduce("", {
     ...
     func validateWordValue(keyToValidate : String, validateObject : PhotoArtValidate) -> Bool{
        var check = true
        if(keyToValidate == "Word"){
            //check whether the input has the word 
            
            if(finalExtractedText.lowercased().range(of: validateObject.word) != nil){
                check = true
            }
            else{
                check = false
